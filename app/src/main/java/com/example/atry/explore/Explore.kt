...
fun CategoryCardItem(category: String, onCardClick: () -> Unit) {
    Card(
        ...
        Box(
            ...
            // Adding the image as the background
            Image(
                painter = painterResource(id = R.drawable.photo2), // Replace with your image resource
                contentDescription = null,
                modifier = Modifier.fillMaxSize(),
                contentScale = ContentScale.Crop // Scale the image to cover the entire box
            )
            // The text will be on top of the image
            Text(text = category,color=Color.White, style = TextStyle(fontWeight = FontWeight.Bold), fontSize = 20.sp)
        }
    }
}
